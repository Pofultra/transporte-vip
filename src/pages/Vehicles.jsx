...
function Vehicles() {
  ...
  return (
    <section id="vehicles" className="bg-black text-white py-8 relative overflow-hidden">
      <div className="container mx-auto px-4 text-center">
        {/* Título */}
        <h2 className="text-2xl sm:text-3xl font-bold text-gold mb-6 uppercase">{t.title}</h2>
        ...
        >
          ...
          <div className="absolute bottom-[15px] left-[15px] right-[15px] bg-black bg-opacity-50 p-3 rounded-md">
            <h3 className="text-lg sm:text-xl font-semibold text-white drop-shadow-lg">{vehicles[currentIndex].name}</h3>
            <div className="flex items-center space-x-3 mt-1 text-white">
              <span className="text-sm flex items-center">
                <i className="ri-user-line mr-1"></i> {vehicles[currentIndex].passengers} {t.passengers}
              </span>
              <span className="text-sm flex items-center">
                <i className="ri-settings-2-line mr-1"></i> {vehicles[currentIndex].transmission}
              </span>
            </div>
          </div>
        </motion.div>
        ...
        <div className="flex justify-center mt-6 gap-2 overflow-x-auto pb-4">
          {vehicles.map((vehicle, index) => (
            ...
            >
              ...
              <p className="absolute bottom-1 left-1 text-[10px] sm:text-xs text-white font-bold">{vehicle.name}</p>
            </div>
          ))}
        </div>
      </div>
